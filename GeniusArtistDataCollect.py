...
class GeniusArtistDataCollect:
    ...
    def get_album_tracks(self, album):
        ...
        tracks = []
        results = sp.album_tracks(albumSp['id'])
        tracks.extend(results['items'])
        while results['next']:
            results = sp.next(results)
            tracks.extend(results['items'])
        tracks = list(map(lambda x: x['name'].lower(), tracks))
        return tracks 
    ...
    def get_artist_songs(self):
        ...
        #df['InAnAlbum'] = df['Album'].apply(lambda a: self.is_track_in_an_album(a, self.album))
